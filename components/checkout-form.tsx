...
export function CheckoutForm() {
  ...
  // Shipping logic: 1 unit = base rate only; multiple units = regular + (totalWeight × minimum per kg), capped at maximum
  const totalWeight = items.reduce(
    (sum, item) => sum + (item.weight ?? 1) * item.quantity,
    0,
  );
  const useWeightBased = totalItems > 1; // Same product qty 2+ or multiple different products
  const insideCost = useWeightBased
    ? Math.min(
        Math.round((settings.insideDhakaShipping + (totalWeight - 1) * settings.minimumShippingCostInsideDhaka) * 100) / 100,
        settings.maximuxShippingCostInsideDhaka,
      )
    : settings.insideDhakaShipping;
  const outsideCost = useWeightBased
    ? Math.min(
        Math.round((settings.outsideDhakaShipping + (totalWeight - 1) * settings.minimumShippingCostOutsideDhaka) * 100) / 100,
        settings.maximuxShippingCostOusideDhaka,
      )
    : settings.outsideDhakaShipping;
  const shippingCost = deliveryLocation === 'inside' ? insideCost : outsideCost;
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                >
                  ...
                  <div className="text-sm text-muted-foreground">
                    Delivery Charge:{' '}
                    <span className="text-foreground font-medium">৳{insideCost}</span>
                    {/* {useWeightBased && (
                      <span className="block mt-0.5 text-xs text-muted-foreground/80">
                        ৳{settings.insideDhakaShipping} + {Number(totalWeight.toFixed(2))} kg × ৳{settings.minimumShippingCostInsideDhaka}/kg
                      </span>
                    )} */}
                  </div>
                  {deliveryLocation === 'inside' && (
                ...
                >
                  ...
                  <div className="text-sm text-muted-foreground">
                    Delivery Charge:{' '}
                    <span className="text-foreground font-medium">৳{outsideCost}</span>
                    {/* {useWeightBased && (
                      <span className="block mt-0.5 text-xs text-muted-foreground/80">
                        ৳{settings.outsideDhakaShipping} + {Number(totalWeight.toFixed(2))} kg × ৳{settings.minimumShippingCostOutsideDhaka}/kg
                      </span>
                    )} */}
                  </div>
                  {deliveryLocation === 'outside' && (
